...
const Hero = () => {
  ...
  const [isTypeSelected, setIsTypeSelected] = useState(false); // Track if "type" is selected
  

  // Define a function to fetch brewery data based on search criteria
  function searchBreweries(searchType, searchValue, typeValue) {  // Define the API endpoint URL

    let apiUrl = "";
  // Define the query parameters based on the search criteria
  let params = {
    per_page: 4, // Number of results per page (you can adjust this as needed)
  };

  if (searchType === 'by_city') {
    params.by_city = searchValue;
  } else if (searchType === 'by_name') {
    params.by_name = searchValue;
  } else if (searchType === 'by_type') {
    params.by_type = typeValue;
  }

  if(selectedOption.value==="by_type"){


   apiUrl = `https://api.openbrewerydb.org/v1/breweries?${searchType}=${selectedTypeOption.value}&per_page=${params.per_page} `;

  }
  else{
     apiUrl = `https://api.openbrewerydb.org/v1/breweries?${searchType}=${breweryName}&per_page=${params.per_page} `;
  ...
  fetch(apiUrl)
    ...
    .then(data => {
      ...
      console.log(data);
      setBreweryInfo(data)
      // You can display the results on your webpage or perform further processing
    })
    .catch(error => {
...
const handleSelectChange = (selected) => {
  ...
  setIsTypeSelected(selected.value === "by_type");
};

  const handleSelectTypeChange = (selected) => {
  ...
  <div className="overflow-hidden ">
    <div className="max-w-[85rem] pb-64 mx-auto px-4 sm:px-6 lg:px-8 py-36">
      <div className="relative mx-auto max-w-4xl grid space-y-5 sm:space-y-10">
        {/* Title */}
        <div className="text-center">
          <p className="text-xs font-semibold text-yellow-500 tracking-wide uppercase mb-3 ">
            The best you'll ever have
          </p>
          <h1 className="text-3xl text-yellow-800 font-bold sm:text-5xl lg:text-6xl lg:leading-tight ">
            Search local Breweries  <br />
            <span className="text-yellow-500">near you</span>
          </h1>
        </div>
        {/* End Title */}
        {/* Avatar Group */}
        ...
        {/* Form */}
        <form>
          <div className="mx-auto max-w-2xl sm:flex sm:space-x-3 p-3 bg-yellow-300 border rounded-lg shadow-lg shadow-gray-100  dark:border-gray-700 dark:shadow-gray-900/[.2]">
            <div className="pb-2 sm:pb-0 sm:flex-[1_0_0%]">
              <label
                htmlFor="hs-hero-name-1"
                className="block text-sm font-medium "
              >
                <span className="sr-only">Brewery</span>
              </label>
              <input
  type="text"
  id="hs-hero-name-1"
  className="py-3 px-4 block w-full border-transparent rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 sm:p-4  dark:border-transparent"
  placeholder="Brewery"
  value={breweryName}
...
/>
            ...
            <div className="pt-2 sm:pt-0 sm:pl-3 border-t border-gray-200 sm:border-t-0 sm:border-l sm:flex-[1_0_0%] dark:border-gray-700">
              ...
              {/* React select- select by city, name , by type */}
            <Select
            ...
            {selectedOption!==null && selectedOption.value ==="by_type"  && <div className="pt-2 sm:pt-0 sm:pl-3 border-t border-gray-200 sm:border-t-0 sm:border-l sm:flex-[1_0_0%] dark:border-gray-700">
              <label
                htmlFor="by-type"
                className="block text-sm font-medium "
              >
         ...
         <Select
            ...
            <button
  className="py-3 px-4 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-yellow-700 text-white hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transition-all text-sm sm:p-4 dark:focus:ring-offset-gray-800"
  onClick={(e) => {
    e.preventDefault();
    if (selectedOption && breweryName && (!isTypeSelected || selectedTypeOption)) {
      searchBreweries(selectedOption.value, breweryName, isTypeSelected ? selectedTypeOption.value : null);
    } else {
...
>
  Search
    ...
    { breweryInfo.length!==0 ?
    <BreweryInfo breweryData={breweryInfo}/> 
  :null 
  } 
